...
ls_config="/usr/bin/ion/local.config.json"
cmd="from sys import path; path.append('/usr/src/myionctrl/'); from mypylib.mypylib import add2systemd; add2systemd(name='ion_http_api', user='${user}', start='${venv_ion_http_api} --logs-level=INFO --host 127.0.0.1 --port 8801 --liteserver-config ${ls_config} --cdll-path ${tonlib_path} --tonlib-keystore /tmp/tonlib_keystore/')"
python3 -c "${cmd}"
systemctl daemon-reload
systemctl restart ion_http_api

# check connection
echo -e "Requesting masterchain info from local ion http api"
sleep 5
