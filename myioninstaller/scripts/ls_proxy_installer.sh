...
ion_src_path=${package_src_path}/ion
proxy_internal_path=${package_src_path}/internal/emulate/lib

proxy_build_path=${bin_path}/${bin_name}
ion_build_path=${proxy_build_path}/ion
db_path=/var/${bin_name}
lib_path=${db_path}/lib

mkdir -p ${lib_path}
mkdir -p ${ion_build_path} && cd ${ion_build_path}
cmake -DCMAKE_BUILD_TYPE=Release -DOPENSSL_FOUND=1 -DOPENSSL_INCLUDE_DIR=${openssl_path}/include -DOPENSSL_CRYPTO_LIBRARY=${openssl_path}/libcrypto.a ${ion_src_path}
make emulator -j ${cpu_number}
cp ${ion_build_path}/emulator/libemulator.so ${lib_path}
cp ${ion_build_path}/emulator/libemulator.so ${proxy_internal_path}
cp ${ion_build_path}/emulator/emulator_export.h ${proxy_internal_path}
