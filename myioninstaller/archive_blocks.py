...
from myioninstaller.utils import is_testnet


def get_block_from_ioncenter(local, workchain: int, shard: int = -9223372036854775808, seqno: int = None, utime: int = None):
    url = f'https://ice.io/api/v2/lookupBlock?workchain={workchain}&shard={shard}'
    if is_testnet(local):
    ...
    if resp.status_code != 200:
        local.add_log(f"Ioncenter API returned status code {resp.status_code}", "error")
        raise Exception(f"Ioncenter API request failed: {resp.text}")
    data = resp.json()
    if not data['ok']:
        local.add_log(f"Ioncenter API returned error: {data.get('error', 'Unknown error')}", "error")
        raise Exception(f"Ioncenter API returned error: {data.get('error', 'Unknown error')}")
    return data['result']
