...
from myioninstaller.utils import StartValidator, StartMyioncore, start_service, stop_service, get_ed25519_pubkey, \
	disable_service, is_testnet, get_block_from_ioncenter
...
def download_bag(local, bag_id: str, downloads_path: str, download_all: bool = True, download_file: typing.Callable = None):
	indexes = []
	local_ts_url = f"http://127.0.0.1:{local.buffer.ion_storage.api_port}"
...
def update_init_block(local, seqno: int):
	...
	with open(local.buffer.global_config_path, 'r') as f:
		config = json.load(f)
	if seqno != 0:
		data = get_block_from_ioncenter(local, workchain=-1, seqno=seqno)
	else:
...
def parse_block_value(local, block: str):
	...
	ts = int(dt.timestamp())
	data = get_block_from_ioncenter(local, workchain=-1, utime=ts)
...
def run_process_hardforks(local, from_seqno: int):
	script_path = os.path.join(local.buffer.mtc_src_dir, 'myioninstaller', 'scripts', 'process_hardforks.py')
	...
	p = subprocess.Popen([
	        ...
	        # "--bin", local.buffer.ion_bin_dir + "validator-engine-console/validator-engine-console",
...
def CreateSymlinks(local):
	...
	myionctrl_file = "/usr/src/myionctrl"
