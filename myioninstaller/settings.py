...
from myionctrl.utils import get_current_user, is_hex
from myioninstaller.archive_blocks import run_process_hardforks, parse_block_value, download_bag, update_init_block, \
...
def FirstNodeSettings(local):
	...
	cmd = f"{validatorAppPath} --threads {cpus} --daemonize --global-config {globalConfigPath} --db {ion_db_dir} --logname {ionLogPath} --verbosity 1"
	cmd += ttl_cmd
...
def download_archive_from_ts(local):
	...
	downloads_path = '/var/ion-work/ts-downloads/'
	...
	url = 'https://archival-dump.ice.io/index/mainnet.json'
	if is_testnet(local):
		url = 'https://archival-dump.ice.io/index/testnet.json'
	...
	archive_dir = local.buffer.ion_db_dir + 'archive/'
	import_dir = local.buffer.ion_db_dir + 'import/'
	...
	stop_service(local, "ion_storage")  # stop TS
	disable_service(local, "ion_storage")
...
def DownloadDump(local):
    ...
    if is_testnet(local):
        url += '_testnet'
    dumpSize = requests.get(url + ".tar.lz.size.txt", timeout=3).text
...
def EnableJsonRpc(local):
	...
	with get_package_resource_path('myioninstaller.scripts', 'jsonrpcinstaller.sh') as jsonrpcinstaller_path:
...
def enable_ion_http_api(local):
	try:
		if not tha_exists():
			do_enable_ion_http_api(local)
	except Exception as e:
		local.add_log(f"Error in enable_ion_http_api: {e}", "warning")
		pass
#end define

def do_enable_ion_http_api(local):
	local.add_log("start do_enable_ion_http_api function", "debug")
	...
	with get_package_resource_path('myioninstaller.scripts', 'ion_http_api_installer.sh') as ion_http_api_installer_path:
		exit_code = run_as_root(["bash", ion_http_api_installer_path, "-u", user])
	if exit_code == 0:
		text = "do_enable_ion_http_api - {green}OK{endc}"
	else:
		text = "do_enable_ion_http_api - {red}Error{endc}"
	color_print(text)
#end define
...
def enable_ls_proxy(local):
	...
	with get_package_resource_path('myioninstaller.scripts', 'ls_proxy_installer.sh') as installer_path:
...
def enable_ion_storage(local):
	...
	with get_package_resource_path('myioninstaller.scripts', 'ion_storage_installer.sh') as installer_path:
		local.add_log(f"Running script: {installer_path}", "debug")
		exit_code = run_as_root(["bash", installer_path, "-u", user])
	if exit_code != 0:
	...
	local.buffer.ion_storage = ion_storage
...
def ConfigureFromBackup(local):
	...
	args = ["python3", "-m", "myioncore", "-e", "enable_btc_teleport"]
	args = ["su", "-l", local.buffer.user, "-c", ' '.join(args)]
	subprocess.run(args)
...
def SetupCollator(local):
	...
	args = ["python3", "-m", "myioncore", "-e", "setup_collator_" + '_'.join(shards)]
	args = ["su", "-l", local.buffer.user, "-c", ' '.join(args)]
	subprocess.run(args)
