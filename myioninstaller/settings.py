...
def DownloadDump(local):
	...
	# download dump
	cmd = "curl -s {url}/dumps/latest.tar.lz | pv | plzip -d -n8 | tar -xC /var/ion-work/db".format(url=url)
	os.system(cmd)
#end define
...
def EnableTonHttpApi(local):
	local.add_log("start EnablePyionv3 function", "debug")
	user = local.buffer.user

	ion_http_api_installer_path = pkg_resources.resource_filename('myioninstaller.scripts', 'ionhttpapiinstaller.sh')
