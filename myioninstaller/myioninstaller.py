...
from myioninstaller.config import GetLiteServerConfig, get_ls_proxy_config
from myioninstaller.node_args import get_node_args
from myioninstaller.utils import GetInitBlock, get_ion_storage_port
from myioncore.utils import dict2b64, str2bool, b642dict, b642hex

from myioninstaller.settings import (
	FirstNodeSettings,
	FirstMyioncoreSettings,
...
from myioninstaller.config import (
	CreateLocalConfig,
	BackupMconfig,
)
...
def Init(local, console):
	...
	# this funciion injects MyPyClass instance
	...
	console.add_item("status", inject_globals(Status), "Print ION component status")
	...
	console.add_item("ion_storage_list", inject_globals(ion_storage_list), "Print result of /list method at Ion Storage API")
...
def Refresh(local):
	...
	bin_dir = "/usr/bin/"
	src_dir = "/usr/src/"
	ion_work_dir = "/var/ion-work/"
	ion_bin_dir = bin_dir + "ion/"
	ion_src_dir = src_dir + "ion/"
	mtc_src_dir = src_dir + "myionctrl/"
	local.buffer.bin_dir = bin_dir
	local.buffer.src_dir = src_dir
	local.buffer.ion_work_dir = ion_work_dir
	local.buffer.ion_bin_dir = ion_bin_dir
	local.buffer.ion_src_dir = ion_src_dir
	local.buffer.mtc_src_dir = mtc_src_dir
	ion_db_dir = ion_work_dir + "db/"
	keys_dir = ion_work_dir + "keys/"
	local.buffer.ion_db_dir = ion_db_dir
	local.buffer.keys_dir = keys_dir
	local.buffer.ion_log_path = ion_work_dir + "log"
	local.buffer.validator_app_path = ion_bin_dir + "validator-engine/validator-engine"
	local.buffer.global_config_path = ion_bin_dir + "global.config.json"
	local.buffer.vconfig_path = ion_db_dir + "config.json"
#end define
...
def set_node_argument(local, args):
	...
	with get_package_resource_path('myioninstaller.scripts', 'set_node_argument.py') as script_path:
...
def Enable(local, args: list):
	if len(args) < 1:
		...
		print("'LSP' - ls-proxy")
		print("'TS' - ion-storage")
		print("Example: 'enable FN'")
		return
	name = args[0]
	if name == "THA":
		CreateLocalConfigFile(local, args)
	args = ["python3", "-m", "myioninstaller", "-u", local.buffer.user, "-e", f"enable{name}"]
	run_as_root(args)
#end define


def DRVCF(local, args):
	args = ["python3", "-m", "myioninstaller", "-u", local.buffer.user, "-e", "drvcf"]
	run_as_root(args)
#end define
...
def ion_storage_list(local, args: list):
	if len(args) > 0:
		api_port = int(args[0])
	else:
		api_port = get_ion_storage_port(local)
		if api_port is None:
			raise Exception('Failed to get Ion Storage API port and port was not provided. Use ion_storage_list [api_port]')
	data = requests.get(f"http://127.0.0.1:{api_port}" + '/api/v1/list', timeout=3)
	if data.status_code != 200:
		raise Exception(f'Failed to get Ion Storage list: {data.text}')
...
def CreateLocalConfigFile(local, args):
	...
	if init_block['rootHash'] is None:
		...
		with open('/usr/bin/ion/global.config.json', 'r') as f:
			config = json.load(f)
	...
	args = ["python3", "-m", "myioninstaller", "-u", user, "-e", "clc", "-i", init_block_b64]
	run_as_root(args)
#end define
...
def Event(local, name):
	...
	if name == "enableJR":
		EnableJsonRpc(local)
	if name == "enableTHA":
		enable_ion_http_api(local)
	if name == "enableLSP":
		enable_ls_proxy(local)
	if name == "enableTS":
		enable_ion_storage(local)
	if name == "clc":
		ix = sys.argv.index("-i")
