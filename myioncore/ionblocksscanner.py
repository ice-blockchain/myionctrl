...
from myioncore.models import Block


class IonBlocksScanner():
	...
	def StatusReading(self):
		while True:
			validatorStatus = self.ion.GetValidatorStatus()
			validatorOutOfSync = validatorStatus.get("outOfSync")
			if self.ion.liteClient.pubkeyPath is None:
				self.working = False
				self.closing = True
				text = "IonBlocksScanner error: local liteserver is not configured, stop thread."
				self.AddLog(text, "error")
				exit()
			if validatorOutOfSync > 20:
				self.working = False
				text = f"IonBlocksScanner warning: local liteserver is out of sync: {validatorOutOfSync}."
	...
	def ScanBlock(self):
		block = self.Try(self.ion.GetLastBlock)
	...
	def ReadBlock(self, block):
		...
		shards = self.Try(self.ion.GetShards, args=(block,))
		for shard in shards:
	...
	def SearchBlock(self, workchain, shardchain, seqno):
		...
		block = self.Try(self.ion.GetBlock, args=(workchain, shardchain, seqno))
		self.StartWithMode(self.NewBlockReaction, args=(block,))
	#end define
	...
	def NewBlockReaction(self, block):
		...
		transactions = self.Try(self.ion.GetTransactions, args=(block,))
	...
	def NewTransReaction(self, trans):
		...
		messageList = self.Try(self.ion.GetTrans, args=(trans,))
