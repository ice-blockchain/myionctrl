...
from myioncore.utils import xhex2hex, ng2g, get_package_resource_path
from myioncore.liteclient import LiteClient
from myioncore.validator_console import ValidatorConsole
from myioncore.fift import Fift
from myioncore.models import (
    Wallet,
    Account,
    Block,
    Trans,
...
class MyIonCore():
	...
	def Refresh(self):
		...
		if liteClient is not None:
			self.liteClient.ion = self # magic
	...
	def parse_stats_from_vc(self, output: str, result: dict):
		for line in output.split('\n'):
			if len(line.split('\t\t\t')) == 2:
				name, value = line.split('\t\t\t')  # https://github.com/ice-blockchain/ion/blob/master/validator-engine-console/validator-engine-console-query.cpp#L648
	...
	def remove_proofs_from_complaint(self, input_file_name: str):
		...
		with get_package_resource_path('myioncore', 'complaints/remove-proofs-v2.fif') as fift_script:
	...
	def SendFile(self, filePath, wallet=None, **kwargs):
		...
		if duplicateApi:
			try:
				self.send_boc_ioncenter(filePath)
			except Exception as e:
				self.local.add_log(f'Failed to send file {filePath} to ioncenter: {e}', 'warning')
		if timeout and wallet:
	...
	def send_boc_ioncenter(self, file_path: str):
		self.local.add_log('Start send_boc_ioncenter function: ' + file_path, 'debug')
		...
		if network_name == 'testnet':
			default_url = 'https://api.testnet.ice.io/http/v2/sendBoc'
		elif network_name == 'mainnet':
			default_url = 'https://api.mainnet.ice.io/http/v2/sendBoc'
		else:
			default_url = None
		...
		if result.status_code != 200:
			self.local.add_log(f'Failed to send boc to ioncenter: {result.content}', 'info')
			return False
		self.local.add_log('Sent boc to ioncenter', 'info')
		return True
	...
	def ElectionEntry(self, args=None):
		...
		if usePool:
			...
			# Send boc file to ION
			resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, pool.addrB64, 4.3)
			...
			# Send boc file to ION
			resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, controllerAddr, 4.03)
			self.SendFile(resultFilePath, wallet)
		else:
	...
	def make_backup(self, election_id: str):
		...
		args = []
		name = f"/myionctrl_backup_elid{election_id}.zip"
	...
	def RecoverStake(self):
		...
		resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, fullElectorAddr, 4)
		self.SendFile(resultFilePath, wallet)
	...
	def PoolRecoverStake(self, poolAddr):
		...
		resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, poolAddr, 4.2)
		self.SendFile(resultFilePath, wallet)
	...
	def PoolProcessUpdateValidatorSet(self, poolAddr, wallet):
		...
		result = self.fift.Run(args)
		resultFilePath = parse(result, "Saved to file ", '\n')
		resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, poolAddr, 4.1)
		self.SendFile(resultFilePath, wallet)
	...
	def PoolWithdrawRequests(self, poolAddr, wallet):
		...
		resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, poolAddr, 40)
		self.SendFile(resultFilePath, wallet)
	...
	def GetOffers(self):
		...
		for offer in rawOffers:
			...
			# Create dict
			# parser from: https://github.com/ice-blockchain/ion/blob/dab7ee3f9794db5a6d32c895dbc2564f681d9126/crypto/smartcont/config-code.fc#L607
			item = Dict()
			item["config"] = Dict()
			item["hash"] = hash
	...
	def GetComplaints(self, electionId=None, past=False):
		...
		for complaint in rawComplaints:
			...
			# Create dict
			# parser from: https://github.com/ice-blockchain/ion/blob/dab7ee3f9794db5a6d32c895dbc2564f681d9126/crypto/smartcont/elector-code.fc#L1149
			item = dict()
	...
	def VoteOffer(self, offer):
		...
		result_file_path = self.SignBocWithWallet(wallet, result_file_path, full_config_addr, 4.5)
	...
	def VoteComplaint(self, electionId, complaintHash):
		...
		resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, fullElectorAddr, 4.5)
		self.SendFile(resultFilePath, wallet)
	#end define
	...
	def get_valid_complaints(self, complaints: dict, election_id: int):
		...
		for complaint in complaints.values():
			...
			if complaint['suggestedFine'] != 101:  # https://github.com/ice-blockchain/ion/blob/5847897b3758bc9ea85af38e7be8fc867e4c133a/lite-client/lite-client.cpp#L3708
				self.local.add_log(f"complaint {complaint['hash_hex']} declined: complaint fine value is {complaint['suggestedFine']} ion", "info")
				continue
			if complaint['suggestedFinePart'] != 0:  # https://github.com/ice-blockchain/ion/blob/5847897b3758bc9ea85af38e7be8fc867e4c133a/lite-client/lite-client.cpp#L3709
				self.local.add_log(f"complaint {complaint['hash_hex']} declined: complaint fine part value is {complaint['suggestedFinePart']} ion", "info")
				continue
	...
	def WithdrawFromPoolProcess(self, poolAddr, amount):
		...
		self.fift.Run(args)
		resultFilePath = self.SignBocWithWallet(wallet, bocPath, poolAddr, 4.35)
		self.SendFile(resultFilePath, wallet)
	#end define
	...
	def CreateLoanRequest(self, controllerAddr):
		...
		self.fift.Run(args)
		resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, controllerAddr, 4.01)
		self.SendFile(resultFilePath, wallet)
	...
	def ReturnUnusedLoan(self, controllerAddr):
		...
		resultFilePath = self.SignBocWithWallet(wallet, fileName, controllerAddr, 4.05)
		self.SendFile(resultFilePath, wallet)
	#end define
	...
	def WithdrawFromControllerProcess(self, controllerAddr, amount):
		...
		self.fift.Run(args)
		resultFilePath = self.SignBocWithWallet(wallet, resultFilePath, controllerAddr, 4.06)
		self.SendFile(resultFilePath, wallet)
	#end define
	...
	def ControllerUpdateValidatorSetProcess(self, controllerAddr, wallet):
		...
		resultFilePath = self.SignBocWithWallet(wallet, fileName, controllerAddr, 4.07)
		self.SendFile(resultFilePath, wallet)
	...
	def ControllerRecoverStake(self, controllerAddr):
		...
		resultFilePath = self.SignBocWithWallet(wallet, fileName, controllerAddr, 4.04)
		self.SendFile(resultFilePath, wallet)
	...
	def get_collator_config_location(self):
		default = 'https://raw.githubusercontent.com/ice-blockchain/ice-blockchain.github.io/main/default_collator_options.json'
	...
	def GetNetworkName(self):
		...
		mainnet_zero_state_root_hash = "4YPrFCebgtI9+Dk9+LwYHDhG+IndIyzsLk31LAl5QBM="
