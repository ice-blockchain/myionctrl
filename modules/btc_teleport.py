...
from myionctrl.console_cmd import add_command, check_usage_args_min_max_len
...
"""
    ...
    def add_daemon(self, user: Optional[str] = None):
        ...
        with get_package_resource_path('myioninstaller', 'scripts/add2systemd.sh') as script_path:
    ...
    def install(self, branch: str, user: Optional[str] = None):
        if user is None:
            user = get_current_user()
        with get_package_resource_path('myionctrl', 'scripts/btc_teleport1.sh') as script_path:
        ...
        with get_package_resource_path('myionctrl', 'scripts/btc_teleport2.sh') as script_path:
