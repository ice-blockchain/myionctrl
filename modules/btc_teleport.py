...
class BtcTeleportModule(MtcModule):
    ...
    def __init__(self, ion, local, *args, **kwargs):
        ...
        self.keystore_path = self.ion.local.buffer.my_work_dir + '/btc_oracle_keystore'
        self.repo_name = 'ion-teleport-btc-periphery'
    ...
    def create_env_file(self, reinit=False):
        ...
        config_path = "/usr/bin/ion/local.config.json"
        if not os.path.exists(config_path):
            config_path = 'https://ice.io/global-config.json'
            warning_text = f"""
WARNING: Could not create local config file. Using global config file ({config_path}).
Please try to create local config file (`myionctrl <<< "installer clcf"`) and update its path in {env_path} and restart 
btc teleport service (`systemctl restart btc_teleport`) or contact validators support. 
"""
            self.local.add_log(warning_text, 'warning')
        text = f"""
COMMON_ION_CONFIG={config_path}
COMMON_ION_CONTRACT_COORDINATOR={self.COORDINATOR_ADDRESS}
ORACLE_STANDALONE_MODE=false
ORACLE_KEYSTORE_PATH={self.keystore_path}
ORACLE_VALIDATOR_ENGINE_CONSOLE_PATH={self.ion.validatorConsole.appPath}
ORACLE_SERVER_PUBLIC_KEY_PATH={self.ion.validatorConsole.pubKeyPath}
ORACLE_CLIENT_PRIVATE_KEY_PATH={self.ion.validatorConsole.privKeyPath}
ORACLE_VALIDATOR_SERVER_ADDR={self.ion.validatorConsole.addr}
...
"""
    ...
    def add_daemon(self, user: str = None):
        ...
        script_path = pkg_resources.resource_filename('myioninstaller', 'scripts/add2systemd.sh')
        if user is None:
    ...
    def install(self, branch: str, user: str = None):
        script_path = pkg_resources.resource_filename('myionctrl', 'scripts/btc_teleport1.sh')
        if user is None:
            user = get_current_user()
        ...
        script_path = pkg_resources.resource_filename('myionctrl', 'scripts/btc_teleport2.sh')
    ...
    def init(self, reinstall=False, branch: str = 'master', user: str = None):
        if os.path.exists(self.src_dir) and not reinstall:
            return
        if self.ion.local.db.get('btcTeleportDisabled'):
    ...
    def run_remove_btc_teleport(args):
        script_path = pkg_resources.resource_filename('myionctrl', 'scripts/remove_btc_teleport.sh')
        return run_as_root(["bash", script_path] + args)

    def get_save_offers(self):
        bname = "saveOffersBtcTeleport"
        save_offers = self.ion.local.db.get(bname)
        if save_offers is None:
            save_offers = dict()
            self.ion.local.db[bname] = save_offers
        return save_offers
    ...
    def get_offers(self):
        ...
        result = self.ion.liteClient.Run(cmd)
        raw_offers = self.ion.Result2List(result)
        raw_offers = raw_offers[0]
        validators = self.ion.GetValidatorsList(fast=True)
        total_weight = 0
        for v in validators:
    ...
    def vote_offer_btc_teleport(self, args):
        if len(args) == 0:
            ...
            return
        wallet = self.ion.GetValidatorWallet(mode="vote")
        validator_key = self.ion.GetValidatorKey()
        validator_pubkey_b64 = self.ion.GetPubKeyBase64(validator_key)
        ...
        for offer_hash in args:
            ...
            request_hash = self.ion.CreateConfigProposalRequest(offer_hash, validator_index)
            validator_signature = self.ion.GetValidatorSignature(validator_key, request_hash)
            path = self.ion.SignProposalVoteRequestWithValidator(offer_hash, validator_index, validator_pubkey_b64,
                                                                 validator_signature)
            path = self.ion.SignBocWithWallet(wallet, path, self.CONFIGURATOR_ADDRESS, 1.5)
            self.ion.SendFile(path, wallet)
    ...
    def remove_btc_teleport(self, args: list):
        ...
        if '--force' not in args:
            if -1 < self.ion.GetValidatorIndex() < self.ion.GetConfig34()['mainValidators']:
