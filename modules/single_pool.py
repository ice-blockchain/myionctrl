...
class SingleNominatorModule(PoolModule):
    ...
    def do_create_single_pool(self, pool_name, owner_address):
        ...
        with get_package_resource_path('myioncore', 'contracts/single-nominator-pool/init.fif') as fift_script:
            with get_package_resource_path('myioncore', 'contracts/single-nominator-pool/single-nominator-code.hex') as code_boc:
    ...
    def do_activate_single_pool(self, pool):
        ...
        result_file_path = self.ion.SignBocWithWallet(validator_wallet, pool.bocFilePath, pool.addrB64_init, 4, boc_mode=boc_mode)
