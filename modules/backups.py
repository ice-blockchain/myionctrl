...
class BackupModule(MtcModule):
    ...
    def run_create_backup(args, user: Optional[str] = None):
        ...
        with get_package_resource_path('myionctrl', 'scripts/create_backup.sh') as backup_script_path:
    ...
    def run_restore_backup(args, user: Optional[str] = None):
        if user is None:
            user = get_current_user()
        with get_package_resource_path('myionctrl', 'scripts/restore_backup.sh') as restore_script_path:
