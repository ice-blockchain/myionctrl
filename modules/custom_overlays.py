...
from myioncore.utils import hex2base64
from myionctrl.console_cmd import add_command, check_usage_two_args, check_usage_one_arg
...
class CustomOverlayModule(MtcModule):
    ...
    def add_custom_overlay(self, args):
        ...
        with open(path, 'r') as f:
            config = json.load(f)
        self.ion.set_custom_overlay(args[0], config)
    ...
    def list_custom_overlays(self, args):
        if not self.ion.get_custom_overlays():
            color_print("{red}No custom overlays{endc}")
            return
        for k, v in self.ion.get_custom_overlays().items():
    ...
    def delete_custom_overlay(self, args):
        if not check_usage_one_arg("delete_custom_overlay", args):
            return
        if '@validators' in self.ion.get_custom_overlays().get(args[0], {}):
            self.ion.delete_custom_overlay(args[0])
    ...
    def check_node_eligible_for_custom_overlay(self, config: dict):
        vconfig = self.ion.GetValidatorConfig()
    ...
    def delete_custom_overlay_from_vc(self, name: str):
        result = self.ion.validatorConsole.Run(f"delcustomoverlay {name}")
        return 'success' in result

    def add_custom_overlay_to_vc(self, config: dict):
        if not self.check_node_eligible_for_custom_overlay(config):
            self.ion.local.add_log(f"Node has no adnl address required for custom overlay {config.get('name')}", "debug")
            return False
        self.ion.local.add_log(f"Adding custom overlay {config.get('name')}", "debug")
        path = self.ion.tempDir + f'/custom_overlay_{config["name"]}.json'
        with open(path, 'w') as f:
            json.dump(config, f)
        result = self.ion.validatorConsole.Run(f"addcustomoverlay {path}")
        return 'success' in result

    def custom_overlays(self):
        config = self.get_default_custom_overlay()
        if config is not None:
            self.ion.set_custom_overlay('default', config)
        self.deploy_custom_overlays()

    def deploy_custom_overlays(self):
        result = self.ion.validatorConsole.Run("showcustomoverlays")
        ...
        for name in names:
            ...
            if suffix.startswith('elid') and suffix.split('elid')[-1].isdigit():  # probably election id
                ...
                if el_id not in (current_el_id, next_el_id):
                    self.ion.local.add_log(f"Overlay {name} is not in current or next election, deleting", "debug")
                    self.delete_custom_overlay_from_vc(name)  # delete overlay if election id is not in current or next election
                    continue

            if pure_name not in self.ion.get_custom_overlays():
                self.ion.local.add_log(f"Overlay {name} ({pure_name}) is not in mtc db, deleting", "debug")
                self.delete_custom_overlay_from_vc(name)  # delete overlay if it's not in mtc db

        for name, config in self.ion.get_custom_overlays().items():
            if name in names:
                continue
            if '@validators' in config:
    ...
    def get_default_custom_overlay(self):
        if not self.ion.local.db.get('useDefaultCustomOverlays', True):
            return None
        network = self.ion.GetNetworkName()
        default_url = 'https://ice-blockchain.github.io/fallback_custom_overlays.json'
        url = self.ion.local.db.get('defaultCustomOverlaysUrl', default_url)
        resp = requests.get(url, timeout=3)
        if resp.status_code != 200:
            self.ion.local.add_log(f"Failed to get default custom overlays from {url}", "error")
            return None
        config = resp.json()
