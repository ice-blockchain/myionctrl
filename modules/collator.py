...
from myioncore.utils import b642hex, signed_int_to_hex64, shard_prefix_len, hex_shard_to_int, shard_prefix, shard_is_ancestor
from myionctrl.console_cmd import check_usage_args_min_len, add_command, check_usage_no_args, check_usage_args_lens
...
class CollatorModule(MtcModule):
    ...
    def add_validator_to_collation_wl(self, args: list):
        if not check_usage_args_min_len("add_validator_to_collation_wl", args, 1):
            return
        self.ion.validatorConsole.Run("collator-whitelist-enable 1")
    ...
    def disable_collation_validator_wl(self, args: list):
        if not check_usage_no_args("disable_collation_wl", args):
            return
        result = self.ion.validatorConsole.Run("collator-whitelist-enable 0")
        if 'success' not in result:
