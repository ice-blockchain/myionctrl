...
from myionctrl.console_cmd import add_command, check_usage_one_arg, check_usage_args_min_max_len
...
from myioninstaller.archive_blocks import download_blocks
from myioninstaller.utils import get_ion_storage_port
...
def Init(local, ion, console, argv):
	...
	with get_package_resource_path('myionctrl', 'resources/translate.json') as translate_path:
	...
	module = BtcTeleportModule(ion, local)
	module.add_console_commands(console)
	...
	if ion.using_validator() or ion.using_collator():
	...
	if ion.using_collator():
		...
		module = CollatorModule(ion, local)
		module.add_console_commands(console)
...
def pre_up(local: MyPyClass, ion: MyIonCore):
	try:
		check_myionctrl_update(local)
...
def Update(local, args):
	...
	# Run script
	with get_package_resource_path('myionctrl', 'scripts/update.sh') as update_script_path:
...
def Upgrade(local, ion, args: list):
	if '--btc-teleport' in args:  # upgrade --btc-teleport [branch] [-u <user>]
		...
		upgrade_btc_teleport(local, ion, reinstall=True, branch=branch, user=user)
		return

	author, repo, branch, git_url = check_git(args, default_repo="ion", text="upgrade")
	...
	# Run script
	with get_package_resource_path('myionctrl', 'scripts/upgrade.sh') as upgrade_script_path:
		if git_url:
		...
		exitCode = run_as_root(runArgs)
	if ion.using_validator():
		upgrade_btc_teleport(local, ion)
	if exitCode == 0:
...
def upgrade_btc_teleport(local, ion, reinstall=False, branch: str = 'master', user = None):
	from modules.btc_teleport import BtcTeleportModule
	module = BtcTeleportModule(ion, local)
...
def rollback_to_mtc1(local, ion,  args):
	...
	with get_package_resource_path('myionctrl', 'migrations/roll_back_001.sh') as rollback_script_path:
...
def run_benchmark(ion, args):
	timeout = 200
	with get_package_resource_path('myionctrl', 'scripts/benchmark.sh') as benchmark_script_path:
		with get_package_resource_path('myionctrl', 'scripts/etabar.py') as etabar_script_path:
...
def check_myionctrl_update(local):
...
def check_sync(local, ion):
	...
	if validator_status.initial_sync or ion.in_initial_sync():
...
def warnings(local: MyPyClass, ion: MyIonCore):
...
def PrintStatus(local, ion, args):
	...
	if all_status:
		...
		rootWorkchainEnabledTime_int = local.try_function(ion.GetRootWorkchainEnabledTime)
		...
		offersNumber = local.try_function(ion.GetOffersNumber)
		complaintsNumber = local.try_function(ion.GetComplaintsNumber)
	...
	if all_status:
		PrintIonStatus(local, network_name, startWorkTime, totalValidators, onlineValidators, shardsNumber, offersNumber, complaintsNumber, tpsAvg)
	...
	if all_status and ion.using_validator():
		PrintIonConfig(local, fullConfigAddr, fullElectorAddr, config15, config17)
		PrintTimes(local, rootWorkchainEnabledTime_int, startWorkTime, oldStartWorkTime, config15)
#end define

def PrintIonStatus(local, network_name, startWorkTime, totalValidators, onlineValidators, shardsNumber, offersNumber, complaintsNumber, tpsAvg):
	#tps1 = tpsAvg[0]
	#tps5 = tpsAvg[1]
	#tps15 = tpsAvg[2]
...
def PrintLocalStatus(local, ion, adnlAddr, validatorIndex, validatorEfficiency, validatorWallet, validatorAccount, validator_status, dbSize, dbUsage, memoryInfo, swapInfo, netLoadAvg, disksLoadAvg, disksLoadPercentAvg, fullnode_adnl):
	...
	elif ion.in_initial_sync():  # states have been downloaded, now downloading blocks
	...
	if ion.using_validator() and validator_status.validator_groups_master is not None and validator_status.validator_groups_shard is not None:
	...
	ls_queries = None
	node_stats = local.try_function(ion.get_node_statistics)
	...
	btc_teleport_path = "/usr/src/ion-teleport-btc-periphery/"
...
def PrintIonConfig(local, fullConfigAddr, fullElectorAddr, config15, config17):
...
def SetSettings(local, ion, args):
...
def download_archive_blocks(local, args: list):
	if len(args) < 2:
		color_print("{red}Bad args. Usage:{endc} download_archive_blocks [ion_storage_api_port] <download_path> <from_block_seqno> [to_block_seqno] [--only-master]")
		return
	...
	if api_port is None:
		api_port = get_ion_storage_port(local)
		if api_port is None:
			raise Exception('Failed to get Ion Storage API port and port was not provided')

	# check ion storage is alive
	local_ts_url = f"http://127.0.0.1:{api_port}"

	try:
		requests.get(local_ts_url + '/api/v1/list', timeout=3)
	except Exception as e:
		color_print(f"{{red}}Error: cannot connect to ion-storage at 127.0.0.1:{api_port}: {type(e)}: {e}. "
					f"Make sure `ion_storage` daemon is running or install it via `installer enable TS`.{{endc}}")
		return

	local.buffer.ion_storage = Dict()
	local.buffer.ion_storage.api_port = api_port
	local.buffer.global_config_path = '/usr/bin/ion/global.config.json'
